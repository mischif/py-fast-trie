...
class XFastTrie(object):
    ...
    def _to_int(value, length):
        ...
        :param length: The maximum length of a value in the trie, which is also its maximum depth
    ...
    def _get_closest_leaf(self, value):
        ...
        # and may be larger or smaller than the given value, depending on the leg
            ...
            return ancestor.left if direction == 0 else ancestor.right
    ...
    def add(self, value):
        ...
        for level in reversed(range(self._maxlen - 1)):
            node_value = value >> (self._maxlen - level - 1)
            ...
            if node is None:
                ...
                if last_inserted.parent is None:
                    last_inserted.parent = node
                leg = "left" if last_inserted.value & 1 == 0 else "right"
                setattr(node, leg, last_inserted)

        if self._root.left is None or self._root.left.leaf:
            root_left = self._level_tables[0].get(0)
            self._root.left = root_left or self._get_closest_leaf(0)
        ...
        if self._root.right is None or self._root.right.leaf:
            ...
            self._root.right = root_right or self._get_closest_leaf(2 ** self._maxlen - 1)
    ...
    def remove(self, value):
        ...
        if value not in self._level_tables[-1]:
            ...
            while node.value is not None:
                ...
                # The value being deleted was the only leaf on the left child of this node,
                # so the value's successor must therefore be the smallest value of this node's right subtree
                # and is the correct target for the descendant pointer
                elif left_child is None:
                    node.left = leaf_succ

                # The value being deleted was the only leaf on the right child of this node,
                # so the value's predecessor must therefore be the largest value of this node's left subtree
                # and is the correct target for the descendant pointer
                elif right_child is None:
                    node.right = leaf_pred
        ...
        if root_left is None and root_right is None:
            ...
            self._root.left = self._get_closest_leaf(0)

        elif root_right is None:
            self._root.right = self._get_closest_leaf(-1)
